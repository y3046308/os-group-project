...
volatile int cat_cum = 0;
volatile int mouse_cum = 0;
               ...
               unsigned long catnumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
	...
	while(eating_cats==NumBowls || eating_mice>0 || (cat_cum%NumBowls==0 && cat_cum!=0)){
		cv_wait(wait_cats, lock[0]);
		cat_wchan = 1;
		if((mouse_cum%NumBowls==0 && mouse_cum!=0) || (eating_mice == 0 && !mouse_wchan)){
			break;
		}
	}
          ...
          unsigned long mousenumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
	...
	lock_acquire(lock[0]);
	while(eating_mice==NumBowls || eating_cats>0 || (mouse_cum%NumBowls==0 && mouse_cum!=0)){
		cv_wait(wait_mice, lock[0]);
		mouse_wchan = 1;
		if((cat_cum%NumBowls==0&&cat_cum!=0) || (!cat_wchan &&eating_cats == 0)){// eating_cats ==0){
			break;
		}
	}
...
#if OPT_A1
  //fair_cat = sem_create("faircat", NumBowls);
  //fair_mouse = sem_create("fairmouse", NumBowls);
